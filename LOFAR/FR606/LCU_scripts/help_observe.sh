...
{
   echo ''
   echo "help_observe.sh - script displaying beamctl commands for LOFAR single station LCU."
   echo ''
   echo "Usage: help_observe.sh <lba/hba/lba8/hba8> <start_subband> <psr> <pulsar name>"
   echo "   or: help_observe.sh <lba/hba/lba8/hba8> <start_subband> <radec> <ra> <dec>"
   echo ''
   echo "where:"
   echo "      lba/hba/lba8/hba8 - LOFAR antennas configuration (lba8/hba8 - RCUs must be set to send 8-bit data)."
   echo "      start_subband     - start subband for allocating 244 consecutive subbands (not parsed in lba8/hba8 mode)."
   ...
   echo ''
   echo "e.g. help_observe.sh hba 220 psr B0329+54"
   echo "e.g. help_observe.sh hba8 psr B0329+54"
   echo "e.g. help_observe.sh lba 157 radec 21:45:50.46412 -07:50:18.4399"
   echo "e.g. help_observe.sh lba8 radec 21:45:50.46412 -07:50:18.4399"
   echo ''
   exit
}
...
if [ -z "${3}" ] || [ ${1} = "-h" ] ; then
   ...
   #echo ${antennas}
   if [ ${antennas} = "LBA" -o ${antennas} = "HBA" ] ; then # check if the name of antennas configuration is valid
      startsubband=${2}
      #echo ${startsubband}
   ...
   elif [ ${antennas} = "HBA8" -o ${antennas} = "LBA8" ] ; then # ...in 8 bit HBA mode
      mode=${2}
      mode=`echo ${mode} | awk '{print toupper($0)}'` # change argument letters to upper-case
      if [ ${mode} = "PSR" ] ; then # check if PSR...
