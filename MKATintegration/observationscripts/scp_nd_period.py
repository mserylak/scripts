...
#                     offtime=cycle_length- ontime
                    on_fraction = cycle_on/100.
#                     user_logger.info("Setting up noise source pattern: %.3f s on and %.3f s off, period %.3f s" % (ontime, offtime, opts.track))
                    user_logger.info("Setting up noise source pattern: %.5f fraction on over period %.2f s" % (on_fraction, cycle_length))
#                     session.fire_noise_diode('coupler', on=ontime, off=offtime, align=False, period=cycle_length)
                    kat.ants.req.dig_noise_source('now', on_fraction, cycle_length)
                    user_logger.info('Tracking for %.2f seconds' % opts.track)
                    session.label('track')
                    session.track(target, duration=opts.track)
